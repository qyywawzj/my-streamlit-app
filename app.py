...
def load_image_from_url(url, width=350):
    try:
        ...
        placeholder = Image.new('RGB', (width, 200), color='#f0f0f0')
        return placeholder
...
st.markdown("### 包含前20道菜：汤、粥、菜、泥、甜点")
...
if st.sidebar.button("🔍 搜索菜谱", type="primary", use_container_width=True):
            ...
            for name, rec in filtered:
                with st.expander(f"🍲 {name} | {rec['category']} | {rec['time']}", expanded=True):
                    ...
                    with col2:
                        ...
                        st.markdown(f"**📊 营养**：{rec['nutrition']}")
...
st.markdown("### 包含后20道菜：汤、粥、饭、菜、甜点")
