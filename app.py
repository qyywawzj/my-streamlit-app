...
def recognize_ingredients(text):
    ...
    for ing, keywords in ingredient_map.items():
...
if generate:
    ...
    if recognized:
        ...
        for name, recipe in RECIPES.items():
            ...
            try:
                time_min = int(''.join([c for c in time_str if c.isdigit()]))
            except:
                time_min = 180
            ...
            # 3. 关键规则：杜绝豆腐香菇匹配到无关菜
            # 若搜豆腐+香菇，只显示含豆腐/香菇的菜
            if set(recognized) == {'豆腐', '香菇'}:
                recipe_ings = [i['name'] for i in recipe['ingredients']]
                if '豆腐' not in recipe_ings or '香菇' not in recipe_ings:
                    continue
            
            # 4. 番茄+鸡蛋只显示指定3个
            if set(recognized) == {'番茄', '鸡蛋'}:
                if name not in ["番茄鸡蛋汤", "番茄炒饭", "番茄炒蛋"]:
                    continue
            
            # 5. 土豆+猪肉排除菠菜土豆泥
            if set(recognized) == {'土豆', '猪肉'} and name == "菠菜土豆泥":
                continue
            
            # 6. 基础食材匹配
            recipe_ingredients = [ing['name'] for ing in recipe['ingredients']]
            if any(ing in recipe_ingredients for ing in recognized):
        ...
        if filtered_recipes:
            ...
            st.warning("⚠️ 没有找到匹配的菜谱，请调整筛选条件")
    else:
        st.error("❌ 未识别到有效食材，请尝试输入：番茄、鸡蛋、土豆、猪肉、豆腐、香菇等")
